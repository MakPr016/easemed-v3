...
export async function POST(req: Request) {
  ...
  try {
    const { documentId, title, deadline, data } = await req.json();
    ...
    // 2. Get Hospital ID linked to this user
    ...
    // 3. Insert RFQ (Correcting the column mapping)
    const { data: rfq, error } = await supabase
      ...
      .insert({
        ...
        status: "draft",
        metadata: data,
        created_by: user.id, // <--- CHANGED: Was 'user_id', now 'created_by' matches your schema
      })
      .select()
      .single();
