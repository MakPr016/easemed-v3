...
Loads extracted RFQ medicines from data/ folder and maps them to vendors
...
def load_vendor_index() -> Dict[str, Any]:
    ...
    path = os.path.join(_get_project_root(), 'data', 'master_index.json')
...
def list_extracted_documents() -> List[Dict[str, Any]]:
    """List all extracted JSON documents from data/extracted/."""
    pattern = os.path.join(_get_project_root(), 'data', 'extracted', '*_extracted.json')
    docs = []
...
def load_document_medicines(doc_id: str) -> List[Dict[str, Any]]:
    ...
    path = os.path.join(_get_project_root(), 'data', 'extracted', f'{doc_id}_extracted.json')
